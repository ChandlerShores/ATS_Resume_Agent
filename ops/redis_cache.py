...
class JDSignalCache:
    ...
    def get(self, jd_hash: str) -> JDSignals | None:
        ...
        try:
            ...
            logger.warn(
                stage="redis_cache", msg=f"Failed to get cached JD signals: {e}", jd_hash=jd_hash
            )
        return None
    ...
    def set(self, jd_hash: str, signals: JDSignals, ttl: int | None = None) -> bool:
        ...
        try:
            ...
            client.setex(f"jd_signals:{jd_hash}", cache_ttl, json.dumps(data))
            return True
        except Exception as e:
            from ops.logging import logger

            logger.warn(
                stage="redis_cache", msg=f"Failed to cache JD signals: {e}", jd_hash=jd_hash
            )
            return False
    ...
    def clear(self, jd_hash: str) -> bool:
        ...
        try:
            ...
            logger.warn(
                stage="redis_cache", msg=f"Failed to clear cached JD signals: {e}", jd_hash=jd_hash
            )
            return False
