...
class CostController:
    ...
    def check_cost_limit(self, model: str = "default") -> tuple[bool, str]:
        ...
        if current_cost + estimated_cost > self.daily_limit:
            return False, f"Daily cost limit would be exceeded (${current_cost + estimated_cost:.2f} > ${self.daily_limit})"
    ...
    def track_request(self, model: str = "default", actual_cost: Optional[float] = None):
        ...
        # Track cost
        cost = actual_cost if actual_cost is not None else self.cost_per_request.get(model, self.cost_per_request["default"])
    ...
    def is_approaching_limit(self) -> tuple[bool, list[str]]:
        ...
        if stats["cost_percentage"] >= 80:
            warnings.append(f"Cost limit: {stats['cost_percentage']:.1f}% used (${stats['daily_cost']:.2f}/${stats['cost_limit']})")
        
        if stats["request_percentage"] >= 80:
            warnings.append(f"Request limit: {stats['request_percentage']:.1f}% used ({stats['daily_requests']}/{stats['request_limit']})")
