...
class SecurityMonitor:
    ...
    def _cleanup_old_data(self, ip: str):
        ...
        self.failed_attempts[ip] = [
            attempt for attempt in self.failed_attempts[ip] if now - attempt["timestamp"] < 3600
        ]

        # Clean up request timestamps older than 1 minute
        self.request_timestamps[ip] = [ts for ts in self.request_timestamps[ip] if now - ts < 60]

        # Clean up suspicious patterns older than 1 hour
        self.suspicious_patterns[ip] = [
            pattern for pattern in self.suspicious_patterns[ip] if now - pattern["timestamp"] < 3600
        ]
    ...
    def log_failed_request(self, ip: str, reason: str, endpoint: str = ""):
        ...
        attempt = {"timestamp": time.time(), "reason": reason, "endpoint": endpoint}
    ...
    def log_suspicious_pattern(self, ip: str, pattern: str, input_type: str = ""):
        ...
        pattern_event = {"timestamp": time.time(), "pattern": pattern, "input_type": input_type}
    ...
    def get_security_stats(self) -> dict[str, any]:
        ...
        return {
            ...
            "total_failed_attempts": sum(
                len(self.failed_attempts[ip]) for ip in self.failed_attempts
            ),
            "total_suspicious_patterns": sum(
                len(self.suspicious_patterns[ip]) for ip in self.suspicious_patterns
            ),
