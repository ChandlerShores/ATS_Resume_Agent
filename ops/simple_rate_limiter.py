...
class SimpleRateLimiter:
    ...
    def _cleanup_old_requests(self):
        ...
        if now - self.last_cleanup > self.cleanup_interval:
            for ip in list(self.requests.keys()):
                self.requests[ip] = [
                    req_time for req_time in self.requests[ip] if now - req_time < 60
                ]
    ...
    def is_allowed(self, ip: str, limit: int = 5) -> tuple[bool, str]:
        ...
        now = time.time()
        recent_requests = [req_time for req_time in self.requests[ip] if now - req_time < 60]
...
def check_rate_limit(request: Request, limit: int = 5) -> None:
    ...
    if not is_allowed:
        raise HTTPException(
            status_code=429,
            detail={"error": "Rate limit exceeded", "message": message, "retry_after": 60},
        )
