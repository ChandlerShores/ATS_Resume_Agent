...
from ops.env import get_settings

# Load settings (validates environment on startup)
...
class ProcessResumeRequest(BaseModel):
    ...
    jd_text: Optional[str] = Field(None, description="Job description text (if URL fails)")
    ...
    settings: Optional[Dict[str, Any]] = Field(None, description="Processing settings")
...
def update_job_status(job_id: str, status: str, progress: int = None, message: str = None, error: str = None):
...
async def root():
    ...
    return {
        "service": "ATS Resume Bullet Revisor API",
        "status": "healthy",
        "version": "1.0.0"
    }
...
if __name__ == "__main__":
    import uvicorn
    # Use settings from environment (security: don't hardcode 0.0.0.0)
