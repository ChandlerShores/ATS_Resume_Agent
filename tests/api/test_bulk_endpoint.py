...
def setup_test_api_keys(monkeypatch):
    ...
    with patch(
        "ops.customer_manager.customer_manager.validate_api_key", side_effect=mock_validate_api_key
    ):
        yield
...
class TestBulkProcessing:
    ...
    def test_bulk_process_request_validation(self):
        ...
        valid_request = {
            ...
            "settings": {"max_len": 30, "variants": 1, "tone": "concise"},
        }
    ...
    def test_bulk_process_request_validation_errors(self):
        ...
        with pytest.raises(ValueError):
            BulkProcessRequest(candidates=[CandidateInput(candidate_id="test", bullets=["test"])])

        # Empty candidates list
        with pytest.raises(ValueError):
            BulkProcessRequest(job_description="test", candidates=[])
        ...
        with pytest.raises(ValueError):
            BulkProcessRequest(job_description="test", candidates=candidates)
    ...
    def test_bulk_process_endpoint_mock(self, mock_state_machine):
        ...
        response = client.post(
            "/api/bulk/process", headers={"X-API-Key": "test_key"}, json=test_request
        )
    ...
    def test_bulk_status_endpoint_structure(self):
        ...
        response = client.get(
            "/api/bulk/status/non-existent-job-id", headers={"X-API-Key": "test_key"}
        )
    ...
    def test_bulk_results_endpoint_structure(self):
        ...
        # Test with non-existent job ID
        response = client.get(
            "/api/bulk/results/non-existent-job-id", headers={"X-API-Key": "test_key"}
        )
        assert response.status_code == 404
        assert "Job not found" in response.json()["detail"]
    ...
    def test_candidate_input_validation(self):
        ...
        long_bullet = "x" * 1001
        candidate = CandidateInput(candidate_id="test", bullets=[long_bullet])
        # Should be truncated to 1000 chars
        assert len(candidate.bullets[0]) == 1000

        # Test empty bullets are filtered out
        candidate = CandidateInput(
            candidate_id="test", bullets=["Valid bullet", "", "   ", "Another valid bullet"]
        )
        assert len(candidate.bullets) == 2
